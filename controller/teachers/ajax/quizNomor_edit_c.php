...
<form action="../../controller/teachers/Quiz.php" method="post">
    ...
    <div class="form-group">
        <label><span class="badge badge-primary">Choice C</span> <span style="color: red;">*</span></label>
        <textarea name="pil_c" id="pil_c" class="form-control" required><?php echo $isi ?></textarea>
    </div>
...
<hr>
    <button type="submit" value="edit_title" class="btn btn-primary" name="edit_pil_c">Update</button>
</form>
...
<script>
    ...
    $('form').submit(function(e){
        var messageLength = CKEDITOR.instances['pil_c'].getData().replace(/<[^>]*>/gi, '').length;
            if( !messageLength ) {
                alert( 'The Chocie C, field is required' );
                e.preventDefault();
            }
    });
</script>