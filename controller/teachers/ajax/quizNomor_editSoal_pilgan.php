...
<form action="../../controller/teachers/Quiz.php" method="post" id="form_edit_soal">
    ...
    <div class="form-group" id="answer_true">
        <label for="jwb_benar"><span class="badge badge-primary">Answer true</span><span style="color: red;"> *</span></label>
        <select name="jwb_benar" class="form-control" id="jwb_benar" required>
            <?php 
                if($jawaban=="pilihan_a"){
            ?>
                <option value="pilihan_a" selected>Choice A</option>
            <?php }else{ ?>
                <option value="pilihan_a">Choice A</option>
            <?php } ?>

            <?php 
                if($jawaban=="pilihan_b"){
            ?>
                <option value="pilihan_b" selected>Choice B</option>
            <?php }else{ ?>
                <option value="pilihan_b">Choice B</option>
            <?php } ?>

            <?php 
                if($jawaban=="pilihan_c"){
            ?>
                <option value="pilihan_c" selected>Choice C</option>
            <?php }else{ ?>
                <option value="pilihan_c">Choice C</option>
            <?php } ?>

            <?php 
                if($jawaban=="pilihan_d"){
            ?>
              <option value="pilihan_d" selected>Choice D</option>
            <?php }else{ ?>
                <option value="pilihan_d">Choice D</option>
            <?php } ?>
        </select>
    </div>
...
<hr>
    <button type="submit" value="edit_title" class="btn btn-primary" name="edit_soal_pilgan">Update</button>
</form>
...
<form action="../../controller/teachers/Quiz.php" method="post" enctype="multipart/form-data" class="hidden" id="form_edit_file">    
    ...
    <?php echo"<input type='hidden' name='jum_file' id='jum_file' value='$jumlah_file'>";?>
        ...
        <br>
    <button type="submit" value="edit_title" class="btn btn-primary" name="update_file_soal_pilgan">Update</button>
</form>
