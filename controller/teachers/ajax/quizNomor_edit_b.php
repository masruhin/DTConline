...
<form action="../../controller/teachers/Quiz.php" method="post">
    ...
    <div class="form-group">
        <label for="Choice A"><span class="badge badge-primary">Choice B</span> <span style="color: red;">*</span></label>
        <textarea name="pil_b" id="pil_b" class="form-control" required><?php echo $isi ?></textarea>
    </div>
...
<hr>
    <button type="submit" value="edit_title" class="btn btn-primary" name="edit_pil_b">Update</button>
</form>
...
<script>
    ...
    $('form').submit(function(e){
        var messageLength = CKEDITOR.instances['pil_b'].getData().replace(/<[^>]*>/gi, '').length;
            if( !messageLength ) {
                alert( 'The Chocie B, field is required' );
                e.preventDefault();
            }
    });
</script>